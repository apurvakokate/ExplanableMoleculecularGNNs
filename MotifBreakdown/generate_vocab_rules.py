...
* method == 'rbrics_old' → CreateMotifVocab plot path: FindrBRICSBonds +
...
# ║ methods (rbrics / rbrics_old / brics).                                    ║
# ║                                                                          ║
...
def _rebrics_pass_tracked(
    ...
    parent molecule (rbrics_full_bonds) misses those cuts and collapses pass 1
    vs pass 2 on most drug-like molecules.
    """
...
def fragment_rbrics_old_tracked(mol: Chem.Mol,
    ...
    """CreateMotifVocab plot path for method='rbrics_old'.
    ...
    0-indexed atom-map tracking (same as replicate_brics_coverage_plot.py).
    """
    if not RBRICS_OK or BreakrBRICSBonds is None:
        raise RuntimeError(
            "method='rbrics_old' requires rBRICS_public.py "
...
def fragment_molecule_tracked(mol: Chem.Mol,
    ...
    """
    if method == 'rbrics_old':
        return fragment_rbrics_old_tracked(mol, orig_smi)

    import brics_rbrics as BR
    n = mol.GetNumAtoms()

    if method == 'rbrics':
        level1 = _rbrics_pass1_tracked(mol, orig_smi)
        ...
        if idx1:
            ...
            all_pieces = [(_canonical_legacy_smarts_from_mol(mol_mapped),
                             set(range(n)))]
    else:
...
def run_dataset(dataset: str, data_root: str, out_dir: Path,
    ...
    # (e.g. "rbrics_old_filter"). When not provided the name is auto-generated.
    ...
    _LEGACY_METHODS = {'rbrics', 'rbrics_old', 'brics'}
    if method == 'rbrics_old' or variant.startswith('rbrics_old'):
    ...
    if method in _LEGACY_METHODS:
        ...
        _n_bad = 0
        if method.startswith(('conservative_ertl_ring', 'ring_mdl', 'mdl_only', 'ring_grow_bpe', 'ring_grow_mdl', 'crush_direct', 'lm_strict', 'lm_bde', 'lm_fg')):
            ...
            # --head_source {ertl,rbrics,none} / --linker_method {mdl,bpe} / --break_fused_rings.
            ...
            if method.startswith(('lm_strict', 'lm_bde', 'lm_fg')):
...
""")
    ...
    p.add_argument('--variant',       default=None,
                   help='Override the auto-generated output variant name '
                        '(e.g. "rbrics_old" or "rbrics_old_filter"). '
