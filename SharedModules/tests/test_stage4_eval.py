...
class Stage4MagePolarity(unittest.TestCase):
    ...
    def test_pipeline_sets_mutag_to_0(self):
        ...
        # silently invert every mutag MAGE attribution.
        r = subprocess.run(
            ['bash', '-c',
             "grep -E 'mutag\\).*mage_official_positive_class 0' run_experiments.sh"],
            cwd=PROJECT, capture_output=True, text=True)
        self.assertEqual(r.returncode, 0,
                         'run_experiments.sh no longer maps mutag → mage positive_class 0')
